...
| 7 `HumanCardModal.jsx` | Closed (399 LoC) | **Reduced (2 Sept 2026)** | 488 lines: the incoming trusted-owner links and the link-on-dog action moved to `human-card/useTrustedOwnerLinks.js` (five hook tests), and the pending-signup link flow #774 added (`handlePhoneTaken` / confirm / cancel) to `usePendingSignupLink.js` (six hook tests, the flow's first). Ratchet lowered to 500. Then (14 Sept 2026) **451 lines**: the delete/archive confirm handlers moved to `useHumanRemoval.js`, and the three confirm dialogs (discard / delete / archive) plus the phone-only New-booking bar became `HumanCardConfirmDialogs.jsx` and `MobileNewBookingBar.jsx` (both component-tested). Ratchet lowered to 465. What is left is the draft-field binding boilerplate and the modal composition itself. |
| 8 `DogCardModal.jsx` | Closed (400 LoC) | Closed | 355 lines. |
| 9 `BookingDetailModal.jsx` | Closed (392 LoC) | **Reduced (2 Sept 2026)** | 487 lines (from 532): the edit-day derivation (date string, day settings, open state, the other live bookings) is `booking-detail/resolveEditDay.ts` (pure, unit-tested) and the debounced edit-mode write is `booking-detail/useBookingDetailAutosave.ts` (hook-tested); the three modal component tests pass unchanged. Ratchet lowered to 500. Then (14 Sept 2026) **472 lines**: the close-attempt/Escape guard is `booking-detail/useBookingDetailClose.ts` and the optimistic reminder-sent override (with its confirmed-always-wins rule as a pure function) is `useReminderSentOverride.ts`, both hook-tested; the three modal component tests pass unchanged. Ratchet lowered to 485. |
...
| 11 `App.jsx` hub | Open, worse (1,025 LoC) | **Closed (2 Sept 2026)** | `App.jsx` is 546 lines (from 1,544): data layer → `useStaffAppData`, drawer session → `useBookingSession`, profile URLs → `useProfileRouting`, routes → `StaffRoutes`, modals → `StaffModals`; `appShell.test.ts` ratchets the line count. The views read shared data from `SalonContext`: `WeekCalendarView` takes 16 props (was 33), `HumansView` 18 (was 26), `DogsView` 18 (was 25). |
...
| `src/supabase/hooks/useDogs.ts` | 931 → 85 | **Split (9 Sept 2026)** | Had the same shape `useHumans` had before Debt 5: pagination + directory controls, mutations and the id-based hydrators in one hook with 12 `useState`s. Now a thin composer over `hooks/dogs/`: `useDogsDirectory` (391 — caches, `search_dogs_directory` paging, realtime, search/sort/filter/letter, archived read), `useDogMutations` (329 — `updateDog`/`addDog`/`deleteDog`), `useDogLookups` (220 — `fetchDogById`, `ensureDogsForHumans`, `ensureDogsByIds`) and `helpers.ts` (102 — shared types + `buildDirectoryDogEntry`). Pure move: the return shape and callback identities are unchanged, and the two component tests (`useDogs`, `useDogs.deferral`) pass untouched. `debtSizeRatchet.test.ts` now caps the facade at 120 lines. |
...
> | Sept 2026 | `auth/LoginPage.jsx`, `auth/ResetPasswordPage.jsx` | `useStaffAuthActions`, pass-through wrappers over `supabase.auth` (reset email, update password, auth-state subscription, session read) (Tier 1.1h) | 0 |
>
> **Allowlist empty (2 September 2026).** The `no-restricted-imports` rule now applies to every component; keep it that way by adding a repository/hook method instead of an `ignores` entry.
