...
**Revised:** 1 September 2026, after review — corrected the leaked-password
behaviour (0.3 and §5), re-diagnosed the empty `supabase` chunk in 0.4, replaced the file-presence coverage claim in 1.7 with
measured coverage, re-scoped the foreign-key index work in 2.4 from an
hours-sized migration to a scoped database task, and noted in 2.1 that the
...
| 0.3 | Enable **leaked-password protection** in Supabase Auth on production (dashboard → Authentication → Password security), and teach both auth paths what it changes. | This is the only advisor finding that is not a deliberate architectural choice. It is an Auth setting, not a migration, so the migration workflow does not apply. Its behaviour has two halves ([Supabase docs](https://supabase.com/docs/guides/auth/password-security)): **password creation and change** with a compromised password is *rejected* (`weak_password`), which reaches [`SetPasswordGate.jsx`](../../src/components/customer/onboarding/SetPasswordGate.jsx) and [`ResetPasswordPage.jsx`](../../src/components/auth/ResetPasswordPage.jsx) as a raw server message today; **sign-in** with a compromised password *succeeds* and returns `data.weakPassword`, which [`useCustomerAuth.js`](../../src/supabase/hooks/useCustomerAuth.js) and [`useAuth.js`](../../src/supabase/hooks/useAuth.js) both ignore. So the item is the setting flip on staging (`btjnxvgkpdbfrrqxvkfj`) then production, plus one small PR: a friendly rejection message on the two set/change paths and a "please change your password" prompt when a sign-in carries the warning. | S (setting) + S (app PR) |
| 0.4 | Explain the empty `supabase` manual chunk in [`vite.config.js`](../../vite.config.js) rather than "fix" it. Instrumenting the build shows the matcher fires for every `@supabase/*` module; the chunk is empty because [`client.ts`](../../src/supabase/client.ts) and `customerClient.ts` only call `createClient()` when `VITE_SUPABASE_URL` and a key exist, so a credential-less build (E2E, fresh clone, CI without secrets) tree-shakes the whole library out. The same build with stub credentials emits a 217 KB chunk. | The first draft of this record misdiagnosed this as a broken matcher. The warning is expected in credential-less builds; what was missing was a comment saying so and a guard test that pins every `@supabase/*` sub-package into the group (supabase-js itself is a thin facade). | S |
| 0.5 | Move [`test-aria.component.test.tsx`](../../test-aria.component.test.tsx) off the repository root. `src/components/shared/AccessibleModal.component.test.tsx` already exists; fold the one assertion in or delete the stray file. | A root-level test file is a leftover from a one-off check and breaks the "tests colocated" convention. | S |
...
| 1.7 | Cover the two thin spots a coverage run exposes. `src/hooks/useSlotDragAndDrop.ts` (drag-to-reschedule) needs a dedicated test; `src/engine/londonTime.ts` needs only its two uncovered branch paths, not a generic test file. | A full `npm run coverage` on 1 September (not the presence or absence of a `*.test.*` file, which the first draft of this record wrongly used as the signal) reports `useSlotDragAndDrop.ts` at 22.2% statements, 12.5% functions, 0% branches, and `londonTime.ts` at 90.9% statements, 100% functions, 62.5% branches through its callers' tests. Drag-and-drop reschedule is a staff write path. | S |
...
| # | What | Why | Cost |
|---|---|---|---|
| 2.1 | **Protect `main`.** Require the `CI` and `Migrations Applied Check` workflows to pass before merge, and require a PR. | `CLAUDE.md` and `docs/migrations.md` are explicit that nothing enforces the "apply the migration before merging" rule; the check already exists, so protection is a settings change, not new engineering. The repository is now **public**, so branch protection rules are available on the free plan; the older documentation's caution about enforcement was written when the repository was private. This is the single highest-leverage item in the plan. | S (needs repo admin) |
...
| 2.4 | Address the **60 unindexed foreign keys** as a scoped piece of database work, not one blanket migration. (a) Inventory the 60 with table size, write rate and the queries that actually join or delete through each key; (b) pick a prioritised subset — the keys on tables that are read or cascaded through at volume (`booking_visits`, `booking_events`, `booking_change_requests`, `booking_financial_ledger`) first, audit-only tables last or never; (c) state the locking model per index: `CREATE INDEX` takes a share lock that blocks writes for the build, `CONCURRENTLY` does not but cannot run inside the transaction the migration runner wraps each file in, so decide per table whether a millisecond build on a small table is acceptable or a non-transactional path is needed; (d) rollback is `DROP INDEX IF EXISTS`, written into the migration header; (e) pgTAP asserts each index exists; (f) staging first via the tranche-1 staging workflow, then production under the standing migration workflow. | Indexes are not free: each adds write cost and the build can lock the table, and Supabase's own guidance is to index from observed query plans rather than every foreign key ([indexing guidance](https://supabase.com/docs/guides/troubleshooting/steps-to-improve-query-performance-with-indexes-q8PoC9)). The first draft of this record called this an hours-sized mechanical change; it is not. Low urgency at 14 dogs/day, so the inventory can wait for the visit model to carry live traffic. | M |
...
3. **Weeks 3–5, in parallel — Tier 2 platform work.** The advisor baseline
   (2.6) is small and independent. The foreign-key index work (2.4) starts
   with its inventory and can wait for live visit traffic to inform the
   subset. Take the major upgrades (2.2) one at a time, Vite 8 last and on
   its own branch with the full E2E matrix.
...
- **Leaked-password protection behaves differently at sign-in and at
  password-set time.** A compromised password still signs the user in, with a
  `weakPassword` warning on the response that neither auth hook currently
  reads; setting or changing to a compromised password is rejected outright.
  Flip the setting without the app change in 0.3 and customers see a raw Auth
  error on the set/change path and nothing at all on sign-in.
