...
| `GOAL-03` consistent capacity | `REQ-CAP-001`, `REQ-CAP-002` | [Capacity authority decision](architecture/decisions/001-postgresql-capacity-authority.md), [capacity reference](capacity-engine.md), [capacity flow](product/user-flows.md#flow-03--a-channel-checks-capacity-and-commits) | #608 | Existing [browser/Deno parity](../src/lib/whatsapp/capacityParity.test.ts), [trigger invariants](../src/engine/capacityTrigger.test.ts) and [extra-slot pgTAP](../supabase/tests/030_extra_slots.test.sql); A1 adds runtime/race proof and B3 adds reason parity. |
