...
import { BRAND, SALON_SLOTS, ALL_DAYS, PRICING, LARGE_DOG_SLOTS } from "./constants/index.ts";
import { computeSlotCapacities } from "./engine/capacity.ts";
import { SAMPLE_BOOKINGS_BY_DAY, SAMPLE_HUMANS, SAMPLE_DOGS } from "./data/sample.ts";
import { supabase } from "./supabase/client.ts";
import { toDateStr } from "./supabase/transforms.ts";
import { useHumans } from "./supabase/hooks/useHumans.ts";
import { useDogs } from "./supabase/hooks/useDogs.ts";
import { useBookings } from "./supabase/hooks/useBookings.ts";
import { useSalonConfig } from "./supabase/hooks/useSalonConfig.ts";
import { useDaySettings } from "./supabase/hooks/useDaySettings.ts";
import type { Dog, Human, Booking, BookingsByDate, DogsMap, HumansMap, DaySettings, SalonConfig, DateInfo } from "./types.ts";
import { Legend } from "./components/ui/Legend.tsx";
import { LoadingSpinner } from "./components/ui/LoadingSpinner.tsx";
import { ErrorBanner } from "./components/ui/ErrorBanner.tsx";
import { SlotRow } from "./components/booking/SlotRow.tsx";
import { DayHeader } from "./components/layout/DayHeader.tsx";
import { ClosedDayView } from "./components/layout/ClosedDayView.tsx";
import { WeekNav } from "./components/layout/WeekNav.tsx";
import { DatePickerModal } from "./components/modals/DatePickerModal.tsx";
import { HumanCardModal } from "./components/modals/HumanCardModal.tsx";
import { DogCardModal } from "./components/modals/DogCardModal.tsx";
import { SettingsView } from "./components/views/SettingsView.tsx";
import { HumansView } from "./components/views/HumansView.tsx";
import { DogsView } from "./components/views/DogsView.tsx";
...
export default function App() {
  ...
  const dates: DateInfo[] = useMemo(() => ALL_DAYS.map((_, i) => {
    const d = new Date(weekStart);
  ...
  const [offlineDogs, setOfflineDogs] = useState<DogsMap>(SAMPLE_DOGS as DogsMap);
  const [offlineHumans, setOfflineHumans] = useState<HumansMap>(SAMPLE_HUMANS as HumansMap);
  const [offlineBookings, setOfflineBookings] = useState<BookingsByDate>(() => buildOfflineBookingsByDate(weekStart));
  const [offlineConfig, setOfflineConfig] = useState<SalonConfig>({
  ...
  const [offlineDaySettings, setOfflineDaySettings] = useState<Record<string, DaySettings>>(() => {
    const settings: Record<string, DaySettings> = {};
  ...
  // Unified callbacks
  const updateDog = isOnline ? sbUpdateDog : useCallback((name: string, updates: Partial<Dog>) => {
    setOfflineDogs(prev => ({ ...prev, [name]: { ...prev[name], ...updates } }));
  }, []);

  const updateHuman = isOnline ? sbUpdateHuman : useCallback((key: string, updates: Partial<Human>) => {
    setOfflineHumans(prev => ({ ...prev, [key]: { ...prev[key], ...updates } }));
  }, []);

  const updateConfig = isOnline ? sbUpdateConfig : useCallback((updater: SalonConfig | ((prev: SalonConfig) => SalonConfig)) => {
    setOfflineConfig(prev => typeof updater === "function" ? updater(prev) : updater);
  }, []);

  const handleAdd = isOnline
    ? useCallback((booking: Booking) => sbAddBooking(currentDateStr, booking), [sbAddBooking, currentDateStr])
    : useCallback((booking: Booking) => {
  ...
  const handleRemove = isOnline
    ? useCallback((bookingId: string | number) => sbRemoveBooking(currentDateStr, bookingId), [sbRemoveBooking, currentDateStr])
    : useCallback((bookingId: string | number) => {
        setOfflineBookings(prev => ({
          ...prev,
  ...
  const handleUpdate = isOnline
    ? sbUpdateBooking
    : useCallback((updatedBooking: Booking, fromDateStr: string, toDateStr: string) => {
        setOfflineBookings(prev => {
          const newState = { ...prev };
  ...
  const handleOverride = isOnline
    ? useCallback((slot: string, seatIndex: number, action: string) => sbSetOverride(currentDateStr, slot, String(seatIndex), action), [sbSetOverride, currentDateStr])
    : useCallback((slot: string, seatIndex: number, action: string) => {
        setOfflineDaySettings(prev => {
          ...
          if (slotOv[key] === action) delete slotOv[key];
          else slotOv[key] = action;
  ...
  const handleDatePick = useCallback((pickedDate: Date) => {
    ...
    const diffWeeks = Math.round((pickedMonday.getTime() - thisMonday.getTime()) / (7 * 24 * 60 * 60 * 1000));
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
        <button type="button" style={{ cursor: "pointer", background: "none", border: "none", padding: 0, textAlign: "left" }} onClick={() => setActiveView("dashboard")} aria-label="Go to dashboard">
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { if (activeView !== "dogs") { e.currentTarget.style.borderColor = BRAND.blue; e.currentTarget.style.color = BRAND.blue; } }}
          onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { if (activeView !== "dogs") { e.currentTarget.style.borderColor = BRAND.greyLight; e.currentTarget.style.color = BRAND.text; } }}>Dogs</button>
          ...
          onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { if (activeView !== "humans") { e.currentTarget.style.borderColor = BRAND.teal; e.currentTarget.style.color = BRAND.teal; } }}
          onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { if (activeView !== "humans") { e.currentTarget.style.borderColor = BRAND.greyLight; e.currentTarget.style.color = BRAND.text; } }}>Humans</button>
          ...
          onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { if (activeView !== "settings") { e.currentTarget.style.borderColor = BRAND.blue; e.currentTarget.style.color = BRAND.blue; } }}
          onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { if (activeView !== "settings") { e.currentTarget.style.borderColor = BRAND.greyLight; e.currentTarget.style.color = BRAND.text; } }}>Settings</button>
          ...
          onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = BRAND.coral; e.currentTarget.style.color = BRAND.white; }}
          onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = BRAND.coralLight; e.currentTarget.style.color = BRAND.coral; }}>Log out</button>
        </div>
      </div>

      {activeView === "settings" && salonConfig ? (
        <SettingsView onBack={() => setActiveView("dashboard")} config={salonConfig} onUpdateConfig={updateConfig as (updater: (prev: SalonConfig) => SalonConfig) => void} />
      ) : activeView === "humans" ? (
              ...
              <div style={{ borderRadius: 14, overflow: "hidden", border: `1px solid ${BRAND.greyLight}`, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
                ...
                <div style={{ padding: "12px 16px", borderTop: `1px solid ${BRAND.greyLight}`, background: BRAND.white, display: "flex", flexDirection: "column", gap: 8 }}>
                  {(currentSettings.extraSlots || []).length > 0 && (
                    ...
                    onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = BRAND.blueDark; }}
                    onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = BRAND.blue; }}>Remove added timeslot</button>
                  )}
                  ...
                  onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = "#D9466F"; }}
                  onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = BRAND.coral; }}>Add another timeslot</button>
                </div>
              </div>
            </>
          ) : (
