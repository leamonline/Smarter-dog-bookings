    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: 20, fontWeight: 800, color: BRAND.text, marginBottom: 4, textAlign: "center" }}>
            Demo Mode
          </div>
          <div style={{ fontSize: 13, color: BRAND.textLight, marginBottom: 20, textAlign: "center" }}>
            Pick a customer to view their dashboard
          </div>

          {demoLoading ? (
            <div style={{ textAlign: "center", color: BRAND.textLight, padding: 20 }}>Loading...</div>
          ) : demoList.length === 0 ? (
            <div style={{ textAlign: "center", color: BRAND.textLight, padding: 20 }}>No customers found</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {demoList.map(h => (
                <button key={h.id} onClick={() => handleDemoSelect(h.id)} style={{
                ...
                onMouseLeave={e => { e.currentTarget.style.borderColor = BRAND.greyLight; e.currentTarget.style.background = BRAND.white; }}>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: BRAND.text }}>{h.name} {h.surname}</div>
          ...
          <button onClick={exitDemo} style={{
            ...
            border: `1px solid ${BRAND.greyLight}`, background: BRAND.white,
            color: BRAND.textLight, fontSize: 13, fontWeight: 600,
  ...
  if (!user) {
    return (
      <CustomerLoginPage
        ...
        phone={phone}
        error={error}
        onDemoMode={() => { setDemoMode(true); loadDemoList(); }}
      />
    );
  }
