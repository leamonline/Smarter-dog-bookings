...
export function BlockedSeatCell({ onClick, span }) {
  return (
    <button
      ...
      className={[
        "tap-target w-full group border-[1.5px] border-slate-200 rounded-xl flex flex-col items-center justify-center gap-0.5 cursor-pointer transition-all duration-150 min-h-[76px] lg:min-h-[80px] bg-slate-50 text-slate-400 hover:border-brand-coral hover:text-brand-coral hover:bg-brand-coral/[0.04] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-coral focus-visible:ring-offset-1 font-[inherit]",
        span ? "col-span-2" : "",
      ].join(" ")}
    >
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="transition-transform duration-150 group-hover:scale-110">
