...
export function MonthTab({ currentDateObj, isActive, onClick }) {
  ...
  return (
    <div
      className={[
        "flex-[1.1] min-w-[64px] md:min-w-[80px] rounded-t-control bg-white text-center border-[1.5px] border-b-0 select-none pb-1.5 cursor-pointer transition-all snap-center shrink-0",
        isActive
    ...
    >
      ...
      <div className="grid grid-cols-7 gap-0 p-[3px_4px] mt-0.5">
        {cells.map((dateStr, i) => {
          ...
          let dotColour;
          if (isToday) {
            dotColour = "var(--color-brand-coral)";
          } else if (hasBookings) {
            dotColour = "#00B8E0";
          } else {
            dotColour = "#E5E7EB";
