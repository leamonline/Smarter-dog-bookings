...
export function DatePickerModal({ currentDate, onSelectDate, onClose, dayOpenState }: Props) {
    ...
    <div ref={trapRef} onClick={onClose} style={{
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="modal-title" onClick={(e: React.MouseEvent<HTMLDivElement>) => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <div id="modal-title" style={{ fontSize: 16, fontWeight: 700, color: BRAND.white }}>{monthName}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", padding: "0 12px 14px", gap: 2 }}>
          {cells.map((d, i) => {
            ...
            return (
              ...
              onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { if (!isSelected(d) && !disabled) e.currentTarget.style.background = BRAND.offWhite; }}
              onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { if (!isSelected(d) && !disabled) e.currentTarget.style.background = isToday(d) ? BRAND.blueLight : "transparent"; }}>
                {d}
              </button>
            );
          })}
