...
import { BRAND, ALERT_OPTIONS } from "../../constants/index.ts";
import { IconEdit, IconTick } from "../icons/index.tsx";
import type { Dog, DogsMap } from "../../types.ts";
...
export function DogCardModal({ dogId, onClose, onOpenHuman, dogs, onUpdateDog }: Props) {
  const trapRef = useFocusTrap(onClose);
  const dog = Object.values(dogs).find(d => d.name === dogId) || { name: dogId, breed: "", age: "", humanId: "", alerts: [], groomNotes: "" };

  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [editNotes, setEditNotes] = useState<string>(dog.groomNotes || "");
  const [editAlerts, setEditAlerts] = useState<string[]>([...(dog.alerts || [])]);

  const [allergyInput, setAllergyInput] = useState<string>(() => {
    const allergy = (dog.alerts || []).find(a => a.startsWith("Allergic to "));
    return allergy ? allergy.replace("Allergic to ", "") : "";
  });
  const [hasAllergy, setHasAllergy] = useState<boolean>(() => (dog.alerts || []).some(a => a.startsWith("Allergic to ")));
    ...
    <div ref={trapRef} onClick={onClose} style={{
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="modal-title" onClick={(e: React.MouseEvent<HTMLDivElement>) => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <div>
            <div id="modal-title" style={{ fontSize: 20, fontWeight: 800, color: BRAND.white }}>{dog.name}</div>
        ...
        <div style={{ padding: "16px 24px 0" }}>
          ...
          {isEditing ? (
            <div style={{ padding: "8px 0", borderBottom: `1px solid ${BRAND.greyLight}` }}>
              ...
              <textarea value={editNotes} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setEditNotes(e.target.value)} style={{...inputStyle, resize: "vertical", minHeight: 60}} />
            </div>
          ) : (
            ...
            <div style={{ marginTop: 16 }}>
              ...
              {hasAllergy && (
                <div style={{ marginTop: 10, display: "flex", justifyContent: "center" }}>
                  <input type="text" placeholder="Allergic to..." value={allergyInput} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAllergyInput(e.target.value)} style={{...inputStyle, textAlign: "center", borderColor: BRAND.coral, borderWidth: 2}} />
                </div>
              )}
            </div>
          ) : (
        ...
        <div style={{ padding: "16px 24px 20px", display: "flex", gap: 10, background: BRAND.offWhite, borderTop: `1px solid ${BRAND.greyLight}`, marginTop: 16 }}>
          {isEditing ? (
            <>
              ...
              }} onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => e.currentTarget.style.background = BRAND.blueDark} onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => e.currentTarget.style.background = BRAND.blue}>
              ...
              }} onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => e.currentTarget.style.background = BRAND.offWhite} onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => e.currentTarget.style.background = BRAND.white}>
                Cancel
              </button>
            </>
          ) : (
