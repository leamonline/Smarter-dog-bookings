...
import type { Human, DogsMap, HumansMap } from "../../types.ts";
...
interface Props {
  ...
  humans: HumansMap;
  dogs: DogsMap;
  onUpdateHuman: (key: string, updates: Partial<Human>) => void;
}

export function HumanCardModal({ humanId, onClose, onOpenHuman, onOpenDog, humans, dogs, onUpdateHuman }: Props) {
  ...
  const [showTrustedSearch, setShowTrustedSearch] = useState<boolean>(false);
  ...
  const handleAddTrusted = (selectedKey: string) => {
    const currentTrusted = human.trustedIds || [];
  ...
  const contactRow = (label: string, value: React.ReactNode, active: boolean) => (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${BRAND.greyLight}` }}>
      <span style={{ fontSize: 13, color: BRAND.textLight }}>{label}</span>
    ...
    <div ref={trapRef} onClick={onClose} style={{
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="modal-title" onClick={(e: React.MouseEvent<HTMLDivElement>) => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <div>
            <div id="modal-title" style={{ fontSize: 20, fontWeight: 800, color: BRAND.white }}>{human.name} {human.surname}</div>
            ...
            <div style={{ marginTop: 10 }}>
              <div style={{ position: "relative" }}>
                ...
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTrustedSearchQuery(e.target.value)}
                  autoFocus
                  style={{
              ...
              {trustedSearchResults.length > 0 && (
                <div style={{ marginTop: 6, border: `1px solid ${BRAND.greyLight}`, borderRadius: 8, overflow: "hidden" }}>
                  {trustedSearchResults.map(([key, h]) => (
                    ...
                    onMouseEnter={(e: React.MouseEvent<HTMLDivElement>) => e.currentTarget.style.background = BRAND.tealLight}
                    onMouseLeave={(e: React.MouseEvent<HTMLDivElement>) => e.currentTarget.style.background = BRAND.white}>
