...
import { IconTick } from "../icons/index.tsx";
import type { SalonConfig, DogSize, ServiceId } from "../../types.ts";

interface Props {
  onBack: () => void;
  config: SalonConfig;
  onUpdateConfig: (updater: (prev: SalonConfig) => SalonConfig) => void;
}

export function SettingsView({ onBack, config, onUpdateConfig }: Props) {
  const [newSlotTime, setNewSlotTime] = useState<string>("");

  const updatePricing = (serviceId: ServiceId, size: DogSize, value: string): void => {
    onUpdateConfig(prev => ({
      ...prev,
      pricing: {
        ...prev.pricing,
  ...
  const updatePickupOffset = (value: string): void => {
  ...
  const removeLargeDogSlot = (time: string): void => {
    onUpdateConfig(prev => {
  ...
  const SettingRow = ({ label, control, border = true }: { label: string; control: React.ReactNode; border?: boolean }) => (
  ...
  const inputStyle: React.CSSProperties = { padding: "8px 12px", borderRadius: 8, border: `1px solid ${BRAND.greyLight}`, fontSize: 13, outline: "none", fontFamily: "inherit", color: BRAND.text, width: 100, textAlign: "right" };
