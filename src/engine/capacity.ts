import { LARGE_DOG_SLOTS } from "../constants/index.ts";
import type { Booking, DogSize, SlotCapacity, CanBookResult } from "../types.ts";
...
export function computeSlotCapacities(bookings: Booking[], activeSlots: string[]): Record<string, SlotCapacity> {
  const seatsMap = getSeatsUsedMap(bookings, activeSlots);
  const capacities: Record<string, SlotCapacity> = {};
...
export function canBookSlot(bookings: Booking[], slot: string, size: DogSize, activeSlots: string[]): CanBookResult {
