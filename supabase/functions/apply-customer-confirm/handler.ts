...
// Exported so the auth-gate tests can drive the handler as a function.
// index.ts wraps this in serve() — the whatsapp-agent shim pattern — because
// importing a module that calls serve() starts an HTTP server, which is why
// these handlers were untestable before the split.
export async function handleApplyCustomerConfirm(req: Request): Promise<Response> {
  if (req.method !== "POST") {
