...
// Exported so handler.test.ts can drive the token gate as a function;
// index.ts wraps this in serve() (the whatsapp-agent / apply-customer-confirm
// shim pattern), because importing a module that calls serve() starts a server.
export async function handleCalendarFeed(req: Request): Promise<Response> {
  try {
    if (req.method !== "GET") {
