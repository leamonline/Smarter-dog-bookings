// calendar-ics/handler.ts — Single-booking .ics download endpoint
...
// Exported so handler.test.ts can drive the gates as a function;
// index.ts wraps this in serve() (the established shim pattern), because
// importing a module that calls serve() starts an HTTP server.
export async function handleCalendarIcs(req: Request): Promise<Response> {
  try {
    // Only allow GET
    if (req.method !== "GET") {
