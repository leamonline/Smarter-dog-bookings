...
create index booking_policy_audit_visit_idx on public.booking_policy_audit(visit_id);
...
create index booking_financial_ledger_visit_idx
  on public.booking_financial_ledger(visit_id);
...
left join lateral (
  select coalesce(sum(coalesce(b.paid_amount, 0) * 100)::int, 0) as gross_service_total_pence
