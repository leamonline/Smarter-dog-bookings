...
--
-- A preview binds exactly what the customer saw: the customer, the source
...
begin
  ...
  if not found
     ...
     or rev.source_fingerprint <> smarter_dog_private.visit_fingerprint(p_visit_id) then
  ...
  if d.state = 'received' then
    if p_paid_deposit_outcome = 'refund' then
      ...
      v_coverage := public.refund_calendar_coverage_for(p_at);
      ...
      values (v_human, p_visit_id, 'refund_due', 1000,
              ...
              'gov.uk/bank-holidays', v_coverage, auth.uid())
...
begin
  ...
  values (p_visit_id, v.human_id, p_kind, p_channel, 'source_deadline_late', 'pending_staff',
          ...
          v_hash,
          case when p_kind = 'reschedule' then v.revision else null end)
...
begin
  ...
  select (select auth.uid()), v_human, p_visit_id, 'cancel', v.revision,
         smarter_dog_private.visit_fingerprint(p_visit_id),
         s.auto_confirm, s.updated_at, public.booking_policy_runtime(),
