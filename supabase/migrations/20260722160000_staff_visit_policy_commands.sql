...
begin
  ...
  elsif (v_resolution->>'blocked')::boolean then
    -- Never start an impossible deadline: raise a staff alert instead.
...
$$;
revoke all on function public.approve_booking_visit(uuid, uuid, text) from public;
revoke all on function public.approve_booking_visit(uuid, uuid, text) from anon;
revoke all on function public.approve_booking_visit(uuid, uuid, text) from authenticated;
grant execute on function public.approve_booking_visit(uuid, uuid, text) to authenticated;
...
begin
  ...
  v_hash := md5(coalesce(p_visit_id::text,'') || '|' || p_customer_reason);
  v_prior := smarter_dog_private.replay_receipt('staff', p_idempotency_key, v_hash);
  if v_prior is not null then
    return v_prior;
  end if;
  ...
  update public.booking_visits set lifecycle_state = 'declined' where id = p_visit_id;
...
$$;
revoke all on function public.decline_booking_visit(uuid, text, uuid) from public;
revoke all on function public.decline_booking_visit(uuid, text, uuid) from anon;
revoke all on function public.decline_booking_visit(uuid, text, uuid) from authenticated;
grant execute on function public.decline_booking_visit(uuid, text, uuid) to authenticated;
...
begin
  ...
  v_hash := md5(coalesce(p_visit_id::text,'') || '|' || p_outcome || '|'
...
$$;
revoke all on function public.record_visit_deposit_outcome(uuid, text, timestamptz, text, uuid, text) from public;
revoke all on function public.record_visit_deposit_outcome(uuid, text, timestamptz, text, uuid, text) from anon;
revoke all on function public.record_visit_deposit_outcome(uuid, text, timestamptz, text, uuid, text) from authenticated;
grant execute on function public.record_visit_deposit_outcome(uuid, text, timestamptz, text, uuid, text) to authenticated;
...
begin
  ...
  v_hash := md5(coalesce(p_visit_id::text,'') || '|' || p_resolution);
  v_prior := smarter_dog_private.replay_receipt('staff', p_idempotency_key, v_hash);
  if v_prior is not null then
    return v_prior;
  end if;
  ...
  if p_resolution = 'refund' then
    ...
    v_coverage := public.refund_calendar_coverage_for(statement_timestamp());
    insert into public.booking_financial_ledger
    ...
    values (v.human_id, p_visit_id, 'refund_due', 1000, p_reason,
            ...
            'deposit_working_days', 'gov.uk/bank-holidays', v_coverage, auth.uid())
    returning id into v_event_id;
...
$$;
revoke all on function public.resolve_visit_deposit_money(uuid, text, uuid, text) from public;
revoke all on function public.resolve_visit_deposit_money(uuid, text, uuid, text) from anon;
revoke all on function public.resolve_visit_deposit_money(uuid, text, uuid, text) from authenticated;
grant execute on function public.resolve_visit_deposit_money(uuid, text, uuid, text) to authenticated;
...
begin
  ...
  v_hash := md5(coalesce(p_visit_id::text,'') || '|' || p_kind);
  v_prior := smarter_dog_private.replay_receipt('staff', p_idempotency_key, v_hash);
  if v_prior is not null then
    return v_prior;
  end if;
...
$$;
revoke all on function public.record_booking_incident(uuid, text, text, uuid) from public;
revoke all on function public.record_booking_incident(uuid, text, text, uuid) from anon;
revoke all on function public.record_booking_incident(uuid, text, text, uuid) from authenticated;
grant execute on function public.record_booking_incident(uuid, text, text, uuid) to authenticated;
...
as $$
  ...
  select public.record_booking_incident(p_visit_id, 'no_show', p_reason, p_idempotency_key);
$$;
revoke all on function public.mark_booking_visit_no_show(uuid, text, uuid) from public;
revoke all on function public.mark_booking_visit_no_show(uuid, text, uuid) from anon;
revoke all on function public.mark_booking_visit_no_show(uuid, text, uuid) from authenticated;
grant execute on function public.mark_booking_visit_no_show(uuid, text, uuid) to authenticated;
...
begin
  ...
  v_hash := md5(coalesce(p_incident_id::text,'') || '|' || p_waived::text);
  v_prior := smarter_dog_private.replay_receipt('staff', p_idempotency_key, v_hash);
  if v_prior is not null then
    return v_prior;
  end if;
...
$$;
revoke all on function public.set_booking_incident_waiver(uuid, boolean, text, uuid) from public;
revoke all on function public.set_booking_incident_waiver(uuid, boolean, text, uuid) from anon;
revoke all on function public.set_booking_incident_waiver(uuid, boolean, text, uuid) from authenticated;
grant execute on function public.set_booking_incident_waiver(uuid, boolean, text, uuid) to authenticated;
...
$$;
revoke all on function public.waive_visit_deposit_requirement(uuid, text, uuid) from public;
revoke all on function public.waive_visit_deposit_requirement(uuid, text, uuid) from anon;
revoke all on function public.waive_visit_deposit_requirement(uuid, text, uuid) from authenticated;
grant execute on function public.waive_visit_deposit_requirement(uuid, text, uuid) to authenticated;
...
begin
  ...
  return jsonb_build_object('contact_event_id', v_id,
                            'outcome_key', 'contact:' || v_id::text);
end;
$$;
revoke all on function public.record_customer_booking_contact(uuid, text, timestamptz, uuid, text) from public;
revoke all on function public.record_customer_booking_contact(uuid, text, timestamptz, uuid, text) from anon;
revoke all on function public.record_customer_booking_contact(uuid, text, timestamptz, uuid, text) from authenticated;
grant execute on function public.record_customer_booking_contact(uuid, text, timestamptz, uuid, text) to authenticated;
...
begin
  ...
  v_hash := md5(coalesce(p_request_id::text,'') || '|' || p_decision);
  v_prior := smarter_dog_private.replay_receipt('staff', p_idempotency_key, v_hash);
  if v_prior is not null then
    return v_prior;
  end if;
  ...
  select * into cr from public.booking_change_requests
   where id = p_request_id and status in ('pending_staff','waiting_customer') for update;
  if not found then
    raise exception 'unknown or already decided request' using errcode = 'P0001';
  end if;
  select * into v from public.booking_visits where id = cr.source_visit_id for update;
...
$$;
revoke all on function public.decide_booking_change_request(uuid, text, text, uuid, boolean) from public;
revoke all on function public.decide_booking_change_request(uuid, text, text, uuid, boolean) from anon;
revoke all on function public.decide_booking_change_request(uuid, text, text, uuid, boolean) from authenticated;
grant execute on function public.decide_booking_change_request(uuid, text, text, uuid, boolean) to authenticated;

comment on function public.record_visit_deposit_outcome(uuid, text, timestamptz, text, uuid, text) is
...
comment on function public.mark_booking_visit_no_show(uuid, text, uuid) is
