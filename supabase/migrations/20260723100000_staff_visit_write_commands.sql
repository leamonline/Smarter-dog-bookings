...
begin
  ...
  if p_expected_revision is not null and v.revision <> p_expected_revision then
    return jsonb_build_object('outcome','blocked','block_reason','stale_review',
                              'visit_id',p_visit_id,'revision',v.revision,'outcome_key',null);
  end if;
  ...
  if d.state = 'received' then
    if coalesce(p_paid_deposit_outcome,'refund') = 'refund' then
      ...
      select rv.due_at, rv.coverage_id into v_due, v_coverage
      ...
      values (v.human_id, p_visit_id, 'refund_due', 1000,
              ...
              v_due, 'deposit', 'deposit_working_days', 'gov.uk/bank-holidays', v_coverage, auth.uid())
  ...
  if coalesce(p_record_incident, false) then
    v_incident := public.record_booking_incident(
      p_visit_id, coalesce(p_incident_kind,'late_cancellation'),
...
begin
  ...
  -- Transfer the deposit state to the replacement, if the source had one.
  select * into d from public.booking_visit_deposits where visit_id = p_visit_id;
  if d.visit_id is not null then
    insert into public.booking_visit_deposits
      (visit_id, origin, state, requirement_decided_at, exemption_reason,
       satisfaction_source, satisfaction_event_id, bank_received_at,
       recorded_at, recorded_by)
    values (v_new_visit, d.origin, d.state, p_at, d.exemption_reason,
            d.satisfaction_source, d.satisfaction_event_id, d.bank_received_at,
            d.recorded_at, d.recorded_by)
...
begin
  ...
  for v_elem in select * from jsonb_array_elements(coalesce(p_changes,'[]'::jsonb)) loop
    update public.bookings b
       set service = coalesce(nullif(v_elem->>'service',''), b.service),
           addons = case when jsonb_typeof(v_elem->'addons') = 'array'
  ...
  -- Bump the visit revision so a stale concurrent editor is rejected.
  update public.booking_visits set updated_at = now() where id = p_visit_id;
