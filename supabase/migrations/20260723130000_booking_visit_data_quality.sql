...
comment on function public.booking_visit_data_quality(uuid) is
  'Explicit authority level for one visit: v1_authoritative | legacy_trustworthy | legacy_incomplete | reconciliation_required | structurally_inconsistent. Read-only — never repairs, mutates or writes. Projections must gate certainty-requiring actions on this rather than on display wording.';
