...
create table public.booking_visits (
  ...
  human_id uuid not null references public.humans(id),
  -- `revision` is the immutable ordinal inside a reschedule/rebook lineage.
  -- `row_revision` is the independently mutable optimistic-lock version.
...
comment on column public.booking_visits.revision is
  'Immutable ordinal within booking_visits.lineage_id; successors must use predecessor revision + 1.';
comment on column public.booking_visits.row_revision is
  'Mutable optimistic-lock version exposed to clients as visitRevision/revision and supplied back as expectedVisitRevision.';
...
-- Every persisted visit-row update advances one optimistic-lock version.
-- Callers may assign row_revision explicitly for readability; the trigger
-- still normalises it to exactly OLD + 1. The lineage ordinal (`revision`) is
-- independent and remains guarded by the lineage constraints below.
create or replace function public.bump_booking_visit_row_revision()
returns trigger
language plpgsql
security definer
set search_path = public, pg_temp
...
$$;
revoke all on function public.bump_booking_visit_row_revision()
  from public, anon, authenticated;

drop trigger if exists booking_visits_row_revision on public.booking_visits;
create trigger booking_visits_row_revision
  before update on public.booking_visits
  for each row execute function public.bump_booking_visit_row_revision();
...
-- A visit's lineage identity, immutable ordinal and predecessor edge are fixed
-- by its INSERT. Only row_revision advances after creation.
...
begin
  if new.lineage_id is distinct from old.lineage_id
      ...
      or new.continues_cancelled_visit_id is distinct from old.continues_cancelled_visit_id then
    raise exception 'visit %: lineage identity and predecessor linkage are immutable', old.id
      using errcode = 'P0001';
  end if;
  return new;
end;
