-- ============================================================
-- validate_booking_capacity: treat No-show as non-occupying
--
-- The third and last part of the status split (20260919090000,
-- 20260919090100). This function is the AUTHORITATIVE capacity gate — the
-- database is the source of truth for the 2-2-1 rule and the daily cap — so it
-- is reproduced here in full, with exactly three predicates changed and every
-- other line byte-identical to the deployed version.
--
-- WHAT CHANGED, and why each one matters:
--
-- 1. The early-exit fast path. It skips revalidation when nothing
--    capacity-relevant changed, EXCEPT when a booking is being un-cancelled,
--    because that re-occupies a seat and must be re-checked. Un-no-showing a
--    booking (a staff correction: "actually they did turn up") re-occupies a
--    seat in exactly the same way. Left as `old.status = 'Cancelled'`, a
--    No-show -> Booked correction would skip capacity validation ENTIRELY and
--    could overbook the slot.
--
-- 2 & 3. The daily-cap guard and its count. Before the split a no-show was a
--    Cancelled row and did not count toward the 14-dogs-per-day cap. Left
--    unchanged, every historical no-show would start consuming a day's
--    allowance and block real bookings.
--
-- WHAT DELIBERATELY DID NOT CHANGE: the `booking_visit_already_cancelled`
-- group guard still tests Cancelled alone. A cancelled group and a dog that
-- failed to turn up are different events, and a no-show sibling should not
-- block reviving another dog in the same group.
--
...
begin
  ...
  -- UNCHANGED: a cancelled group and a no-show dog are different events.
