...
create temp table settings_version_checkpoints (
  label text primary key,
  version_at timestamptz not null
);
insert into settings_version_checkpoints
...
select lives_ok(
  $$ select public.update_booking_rules('{"autoConfirm":false}'::jsonb) $$,
  'a second rules save in the same transaction succeeds');
select cmp_ok(
  (select updated_at from public.booking_policy_settings where singleton),
  '>',
  (select version_at from settings_version_checkpoints where label = 'rules'),
  'each committed rules save advances the settings review version');
...
select cmp_ok(
  (select updated_at from public.booking_policy_settings where singleton),
  '>',
  (select version_at from settings_version_checkpoints where label = 'intake'),
  'the intake writer advances the same settings review version');
select is(
