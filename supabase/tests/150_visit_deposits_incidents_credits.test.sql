...
insert into public.dogs (id, name, breed, size, human_id, custom_price) values
  ('15000000-0000-4000-8000-000000000011', 'Echo', 'Poodle', 'small',
   '15000000-0000-4000-8000-000000000010', null),
  ('15000000-0000-4000-8000-000000000012', 'Foxtrot', 'Beagle', 'small',
   '15000000-0000-4000-8000-000000000010', 27);
...
insert into public.salon_config (pricing)
select '{}'::jsonb
where not exists (select 1 from public.salon_config);
update public.salon_config
set pricing = '{
  "full-groom":{"small":4300},
  "bath-and-brush":{"small":"£39"}
}'::jsonb;

select is(
  smarter_dog_private.booking_guide_price_pence('Full Groom', 'small'),
  4300,
  'the bill price resolver reads integer-pence Settings values'
);
select is(
  smarter_dog_private.booking_guide_price_pence('Bath & Brush', 'small'),
  3900,
  'the bill price resolver tolerates legacy pound-string Settings values'
);

-- ── 3-7: refund working days ───────────────────────────────────────
...
select is(
  (select gross_service_total_pence from public.booking_visit_bill_summary
    where visit_id = '15000000-0000-4000-8000-000000000502'),
  7700,
  'gross value uses booking override, dog usual price and add-ons, never paid_amount');
select is(
  (select amount_due_pence from public.booking_visit_bill_summary
    where visit_id = '15000000-0000-4000-8000-000000000502'),
  6700, 'the final bill is reduced by the deposit exactly once');
