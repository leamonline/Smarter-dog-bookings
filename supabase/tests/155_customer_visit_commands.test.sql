...
select is(
  public.preview_customer_cancel_visit(
    '15500000-0000-4000-8000-000000000501') ->> 'block_reason',
  'policy_not_active',
  'cancellation preview is inert while the policy is inactive');

select is(
  (select count(*)::int
     from smarter_dog_private.booking_change_reviews
    where source_visit_id = '15500000-0000-4000-8000-000000000501'),
  0,
  'an inactive cancellation preview persists no review token');
...
-- Model a token issued by the pre-fix preview so activation can prove the
-- submit boundary rejects it rather than letting it cross runtime states.
insert into smarter_dog_private.booking_change_reviews
  (id, customer_user_id, human_id, source_visit_id, intent, source_revision,
   source_fingerprint, auto_confirm, settings_version, runtime_state, expires_at)
select
  '15500000-0000-4000-8000-000000000902',
  '15500000-0000-4000-8000-000000000003',
  '15500000-0000-4000-8000-000000000010',
  '15500000-0000-4000-8000-000000000501',
  'cancel',
  v.row_revision,
  smarter_dog_private.visit_fingerprint(v.id),
  s.auto_confirm,
  s.updated_at,
  'inactive',
  statement_timestamp() + interval '30 minutes'
from public.booking_visits v
cross join public.booking_policy_settings s
where v.id = '15500000-0000-4000-8000-000000000501'
  and s.singleton;
...
-- Activate only inside this rolled-back fixture, through the same guarded
-- latch used by the policy-rules suite. This keeps public gates and any nested
-- policy command on the real runtime path.
select set_config(
  'request.jwt.claims',
...
-- ── active-runtime behaviour via the private time seam ───────────────

select is(
  smarter_dog_private.cancel_customer_visit_dispatch(
    '15500000-0000-4000-8000-000000000501',
    '15500000-0000-4000-8000-000000000902',
    '15500000-0000-4000-8000-000000000830',
    null, 'refund', statement_timestamp(), 'active') ->> 'block_reason',
  'review_required',
  'a review created while inactive cannot cross activation');

select is(
  (select lifecycle_state from public.booking_visits
    where id = '15500000-0000-4000-8000-000000000501'),
  'active',
  'rejecting the pre-activation review leaves the visit unchanged');

select pg_temp.take_review('settings-stale', '15500000-0000-4000-8000-000000000501');
update public.booking_policy_settings
   set auto_confirm = not auto_confirm,
...
select is(
  smarter_dog_private.cancel_customer_visit_dispatch(
    '15500000-0000-4000-8000-000000000501',
    (select review_id from review_tokens where label = 'settings-stale'),
    '15500000-0000-4000-8000-000000000831',
    null, 'refund', statement_timestamp(), 'active') ->> 'block_reason',
  'review_required',
  'a settings change invalidates an otherwise matching review');

-- A customer withdrawal follows the same aggregate-first lock contract as a
-- staff decision and still produces the expected public outcome.
insert into public.booking_change_requests
  (id, source_visit_id, human_id, kind, channel, reason_code, status, requested_at)
values
  ('15500000-0000-4000-8000-000000000601',
   '15500000-0000-4000-8000-000000000501',
   '15500000-0000-4000-8000-000000000010',
   'cancel', 'website', 'source_deadline_late', 'pending_staff', statement_timestamp());

select is(
  public.withdraw_customer_booking_change_request(
    '15500000-0000-4000-8000-000000000601',
    '15500000-0000-4000-8000-000000000832') ->> 'outcome',
  'change_request_withdrawn',
  'the owner may withdraw an open change request');

select is(
  (select status from public.booking_change_requests
    where id = '15500000-0000-4000-8000-000000000601'),
  'withdrawn',
  'withdrawal makes the request terminal');
...
-- A genuine review token, then a staff write advances the visit row version
-- without changing the booking fingerprint.
...
update public.booking_visits set row_revision = row_revision + 1
 where id = '15500000-0000-4000-8000-000000000501';
select set_config(
  'request.jwt.claims',
  '{"sub":"15500000-0000-4000-8000-000000000003","role":"authenticated"}',
...
select is(
  smarter_dog_private.cancel_customer_visit_dispatch(
    ...
    null, 'refund', statement_timestamp(), 'active') ->> 'block_reason',
  'review_required',
  'a changed visit row revision invalidates an otherwise matching review token');
