...
select throws_ok(
  $$ select public.approve_booking_visit(
       '16000000-0000-4000-8000-000000000501', gen_random_uuid()) $$,
  '42501', null, 'approval is staff-only');
select throws_ok(
  $$ select public.decline_booking_visit(
       '16000000-0000-4000-8000-000000000501', 'no space', gen_random_uuid()) $$,
  '42501', null, 'declining is staff-only');
select throws_ok(
  $$ select public.record_visit_deposit_outcome(
       '16000000-0000-4000-8000-000000000501', 'received', now(), 'ref', gen_random_uuid()) $$,
  '42501', null, 'deposit reconciliation is staff-only');
select throws_ok(
  $$ select public.mark_booking_visit_no_show(
       '16000000-0000-4000-8000-000000000501', 'absent', gen_random_uuid()) $$,
...
-- ── 6-9: approval of a clean customer confirms with no deposit ─────
...
select is(
  public.approve_booking_visit(
    '16000000-0000-4000-8000-000000000501',
...
select is(
  public.approve_booking_visit(
    '16000000-0000-4000-8000-000000000502',
...
select is(
  public.record_visit_deposit_outcome(
    '16000000-0000-4000-8000-000000000502', 'received',
...
select public.approve_booking_visit(
  '16000000-0000-4000-8000-000000000507', '16000000-0000-4000-8000-000000000820');
...
select throws_ok(
  $$ select public.record_visit_deposit_outcome(
       '16000000-0000-4000-8000-000000000507', 'received',
...
select public.approve_booking_visit(
  '16000000-0000-4000-8000-000000000503', '16000000-0000-4000-8000-000000000804');
update public.booking_visit_deposits set state = 'awaiting_payment',
       terms_accepted_at = now()
...
select is(
  public.record_visit_deposit_outcome(
    '16000000-0000-4000-8000-000000000503', 'not_received', null, 'checked',
...
select throws_ok(
  $$ select public.mark_booking_visit_no_show(
       '16000000-0000-4000-8000-000000000504', 'absent',
...
select public.approve_booking_visit(
  '16000000-0000-4000-8000-000000000504', '16000000-0000-4000-8000-000000000807');
select is(
  public.waive_visit_deposit_requirement(
    '16000000-0000-4000-8000-000000000504', 'goodwill for a long-standing customer',
...
select ok(
  (public.mark_booking_visit_no_show(
     '16000000-0000-4000-8000-000000000504', 'did not arrive',
...
select throws_ok(
  $$ select public.record_booking_incident(
       '16000000-0000-4000-8000-000000000504', 'late_cancellation', 'second',
       gen_random_uuid()) $$,
...
select is(
  (public.set_booking_incident_waiver(
     ...
     true, 'salon error', '16000000-0000-4000-8000-000000000809') ->> 'waived'),
...
select public.approve_booking_visit(
  '16000000-0000-4000-8000-000000000505', '16000000-0000-4000-8000-000000000811');
...
select is(
  public.decide_booking_change_request(
    '16000000-0000-4000-8000-000000000601', 'accept',
...
select public.approve_booking_visit(
  '16000000-0000-4000-8000-000000000506', '16000000-0000-4000-8000-000000000813');
insert into public.booking_change_requests
  (id, source_visit_id, human_id, kind, channel, reason_code, status, requested_at)
...
select is(
  (select count(*)::int
     from (select public.decide_booking_change_request(
             '16000000-0000-4000-8000-000000000602', 'decline',
