...
insert into public.booking_refund_calendar_coverage
  (id, calendar_source, calendar_version, covers_from, covers_to, recorded_at)
values
  ('16800000-0000-4000-8000-000000000101', 'tied-short', 'test-1',
   date '2026-01-01', date '2028-12-31', statement_timestamp()),
  ('16800000-0000-4000-8000-000000000102', 'tied-long', 'test-1',
   date '2026-01-01', date '2029-12-31', statement_timestamp());

select is(
  (select calendar_source from public.refund_due_at_verified(now())),
  'tied-long',
  'tied coverage assertions choose the widest deterministic row');

-- ── 5-6: outside verified coverage it fails loudly ─────────────────
...
-- ── 7-10: escalating advance warning ───────────────────────────────
