-- Booking-policy projections: one customer record per visit, deterministic dogs,
...
values (
  ...
  'https://smarterdog.co.uk/terms/projection-172',
  'projection-172',
  repeat('a', 64),
...
values (
  '17200000-0000-4000-8000-000000000102',
...
begin
  insert into public.booking_visits
    (id, lineage_id, human_id, booking_date, lifecycle_state, approval_state,
     confirmation_state, policy_code, requested_at, commercial_eligibility_at,
     eligibility_policy_code, confirmed_at, customer_change_deadline_at,
     runtime_generation, legacy_compat_key, source, terms_publication_id,
     row_revision)
  values (p_id, p_lineage, p_human, v_date, 'active', 'not_required',
          ...
          'website',
          case when p_gen = 'visit_v1'
            then '17200000-0000-4000-8000-000000000101'::uuid
            else null
          end,
          p_row_revision);
  return p_id;
...
insert into public.booking_financial_ledger
  (id, human_id, visit_id, event_kind, amount_pence, reason, idempotency_key,
   recorded_by)
values (
  '17200000-0000-4000-8000-000000000810',
  '17200000-0000-4000-8000-000000000010',
  '17200000-0000-4000-8000-000000000501',
  'deposit_received', 1000, 'projection privacy fixture',
  'projection-172-deposit-received',
  '17200000-0000-4000-8000-000000000001'
);

insert into public.booking_visit_deposits (
  visit_id, origin, state, requirement_reason, requirement_decided_at,
  bank_instruction_id, customer_payment_reference, staff_verification_reference,
  bank_received_at, satisfaction_source, satisfaction_event_id, recorded_at,
  recorded_by, terms_publication_id, terms_accepted_at
) values (
  '17200000-0000-4000-8000-000000000501',
  'visit_v1', 'received', 'three_incidents_12m', statement_timestamp(),
  '17200000-0000-4000-8000-000000000102', 'PROJ172-501',
  'PRIVATE-STAFF-VERIFY-172', statement_timestamp(), 'bank',
  '17200000-0000-4000-8000-000000000810', statement_timestamp(),
  '17200000-0000-4000-8000-000000000001',
  '17200000-0000-4000-8000-000000000101', statement_timestamp()
);
...
values (
  ...
  'visit_v1', 'not_required', statement_timestamp(), 'clean_history'
);
...
select is(
  public.list_customer_booking_visits() -> 0 ->> 'id',
  '17200000-0000-4000-8000-000000000501', 'the customer sees their own visit');

select is(
  public.list_customer_booking_visits() -> 0 ->> 'revision',
  '4',
  'the customer receives the optimistic row revision, not the lineage ordinal');
...
select ok(
  public.list_customer_booking_visits()::text not like '%PRIVATE-STAFF-VERIFY-172%',
  'the fixture''s staff verification reference never reaches the customer projection');
select ok(
...
-- ── 18-19: structured pending changes stay on the source visit ─────
...
values
  ('17200000-0000-4000-8000-000000000701',
   '17200000-0000-4000-8000-000000000501',
   '17200000-0000-4000-8000-000000000010',
   3, 'reschedule', 'website', 'source_deadline_late', 'pending_staff',
   statement_timestamp(), pg_temp.open_day(44),
   '[{"dog_id":"17200000-0000-4000-8000-000000000011","slot":"10:00"}]'::jsonb,
   'projection-destination-hash', 1);

select set_config('request.jwt.claims',
  '{"sub":"17200000-0000-4000-8000-000000000003","role":"authenticated"}', true);

select is(
  public.list_customer_booking_visits() -> 0 -> 'pendingChange' ->> 'requestId',
  '17200000-0000-4000-8000-000000000701',
  'a pending destination change remains nested on its confirmed source visit');

select is(
  public.list_customer_booking_visits() -> 0 -> 'pendingChange',
  jsonb_build_object(
    'requestId', '17200000-0000-4000-8000-000000000701',
    'revision', 3,
    'status', 'pending_staff',
    'staffReviewReason', 'staff_review',
    'requestedBookingDate', pg_temp.open_day(44),
    'slots', jsonb_build_array('10:00'),
    'capacityHeld', false,
    'destinationHash', 'projection-destination-hash',
    'proposalId', null,
    'proposalRevision', null
  ),
  'the pending-change shape carries its revision, destination and honest hold state');

select ok(
  public.list_customer_booking_visits()::text not like '%source_deadline_late%',
  'the internal late-source reason is replaced by a customer-safe review reason');

-- ── 20-24: honest data-quality marking ─────────────────────────────

select is(
  public.booking_visit_data_quality('17200000-0000-4000-8000-000000000501'),
  'v1_authoritative', 'a clean v1 visit is authoritative');

-- A confirmed row cannot become authoritative merely by claiming the v1
-- generation: the mandatory commercial-eligibility and Terms snapshots are
-- part of the authority claim.
select set_config('request.jwt.claims',
  '{"sub":"17200000-0000-4000-8000-000000000001","role":"authenticated"}', true);
insert into public.booking_visits
  (id, lineage_id, human_id, booking_date, lifecycle_state, approval_state,
...
values (
  '17200000-0000-4000-8000-000000000504',
  '17200000-0000-4000-8000-000000000304',
  '17200000-0000-4000-8000-000000000010',
  pg_temp.open_day(50), 'active', 'not_required', 'confirmed',
  'previous_day_1500_v1', statement_timestamp(),
  public.change_deadline_for(
    'previous_day_1500_v1', pg_temp.open_day(50), '08:30'),
  'visit_v1', 'website'
);
insert into public.bookings
  (booking_date, slot, dog_id, size, service, status, visit_id)
values (
  pg_temp.open_day(50), '12:00',
  '17200000-0000-4000-8000-000000000011',
  'small', 'Full Groom', 'Booked',
  '17200000-0000-4000-8000-000000000504'
);

select is(
  public.booking_visit_data_quality('17200000-0000-4000-8000-000000000504'),
  'structurally_inconsistent',
  'a confirmed v1 row missing mandatory snapshots is not called authoritative');
...
select is(
  (select r ->> 'dataQualityNote' from jsonb_array_elements(
    ...
    where r ->> 'id' = '17200000-0000-4000-8000-000000000502'),
...
-- ── 25-26: a malformed child never hides the visit ─────────────────
...
select is(
  (select jsonb_array_length(r -> 'dogs') from jsonb_array_elements(
     public.list_customer_booking_visits()) r
    where r ->> 'id' = '17200000-0000-4000-8000-000000000501'),
...
-- ── 27-29: hidden history preserves pending refunds and stable facts ─
...
insert into public.booking_visits
  (id, lineage_id, human_id, booking_date, lifecycle_state, approval_state,
   confirmation_state, policy_code, requested_at, commercial_eligibility_at,
   eligibility_policy_code, confirmed_at, customer_change_deadline_at,
   runtime_generation, source, cancelled_at, terms_publication_id)
values
  ('17200000-0000-4000-8000-000000000503',
   '17200000-0000-4000-8000-000000000303',
   '17200000-0000-4000-8000-000000000010',
   current_date - 90, 'cancelled', 'not_required', 'confirmed',
   'previous_day_1500_v1', statement_timestamp() - interval '100 days',
   statement_timestamp() - interval '100 days', 'previous_day_1500_v1',
   statement_timestamp() - interval '100 days',
   public.change_deadline_for(
     'previous_day_1500_v1', current_date - 90, '08:30'),
   'visit_v1', 'website', statement_timestamp() - interval '90 days',
   '17200000-0000-4000-8000-000000000101');

insert into public.bookings
  (booking_date, slot, dog_id, size, service, status, visit_id)
values (
  current_date - 90, '08:30',
  '17200000-0000-4000-8000-000000000011',
  'small', 'Full Groom', 'Cancelled',
  '17200000-0000-4000-8000-000000000503'
);
...
values (
  '17200000-0000-4000-8000-000000000503',
  'visit_v1', 'not_required', statement_timestamp() - interval '100 days',
  'clean_history'
);
...
select
  '17200000-0000-4000-8000-000000000801',
  '17200000-0000-4000-8000-000000000010',
  '17200000-0000-4000-8000-000000000503',
  'refund_due', 1000, 'pending visit refund', 'projection-refund-due',
...
select ok(
  exists (
    select 1
    from jsonb_array_elements(public.list_customer_booking_visits(false)) row
    where row ->> 'id' = '17200000-0000-4000-8000-000000000503'
  ),
  'a terminal visit with an unsettled refund remains visible when history is hidden');
...
values
  ('17200000-0000-4000-8000-000000000010',
   '17200000-0000-4000-8000-000000000503',
   'refund_cancelled', 1000, 'test settlement', 'projection-refund-settled',
   '17200000-0000-4000-8000-000000000801');
...
select ok(
  ...
  'the resolved terminal visit obeys the hidden-history switch again');

select set_config('request.jwt.claims',
  '{"sub":"17200000-0000-4000-8000-000000000001","role":"authenticated"}', true);
select public.update_booking_rules(
  '{"customerPortal":{"showHistory":false}}'::jsonb
);
select set_config('request.jwt.claims',
  '{"sub":"17200000-0000-4000-8000-000000000003","role":"authenticated"}', true);
...
select ok(
  not exists (
    select 1
    from jsonb_array_elements(public.list_customer_booking_visits(true)) row
    where row ->> 'id' = '17200000-0000-4000-8000-000000000503'
  ),
  'the caller cannot override the server history switch to reveal resolved history');

select ok(
  (select row -> 'capabilities' -> 'cancel' ? 'deadlinePassed'
     from jsonb_array_elements(public.list_customer_booking_visits()) row
    where row ->> 'id' = '17200000-0000-4000-8000-000000000502'),
  'an uncertain visit still returns the full actionability shape');

-- ── 30-31: reading writes nothing ──────────────────────────────────
...
select (select count(*) from public.booking_visits) v,
       ...
       (select updated_at from public.booking_visits
         where id = '17200000-0000-4000-8000-000000000501') target_updated_at;
...
select is(
  (select updated_at from public.booking_visits
    ...
    = (select target_updated_at from before_counts),
